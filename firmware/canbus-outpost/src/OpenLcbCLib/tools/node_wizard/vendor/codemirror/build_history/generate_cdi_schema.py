...
======================
Reads the OpenLCB CDI XSD and regenerates the CDI_SCHEMA JavaScript block
used by @codemirror/lang-xml in cm-adapter.js.

Usage
-----
  # Print the generated schema to stdout:
  python3 generate_cdi_schema.py --xsd /path/to/cdi.xsd

  # Also patch cm-adapter.js in-place:
  python3 generate_cdi_schema.py --xsd /path/to/cdi.xsd --patch
...
CDI_SCHEMA block.  Run this script whenever the CDI XSD is updated to keep
...
MARKER_START = '/* <<CDI_SCHEMA_START>> */'
MARKER_END   = '/* <<CDI_SCHEMA_END>> */'
    ...
    for el in root.findall(xs('element')):
        ...
        if name == 'cdi':
            ...
            if ct is not None:
                for node in ct:
                    if node.tag in (xs('sequence'), xs('choice'), xs('all')):
                        specs['cdi']['children'] = collect_children_from_container(node, named_types)
                        recurse_into_container(node)
                        break
...
def build_js(specs):
    """
    Convert the specs dict into the CDI_SCHEMA JavaScript block that
    @codemirror/lang-xml understands.
    """

    lines = []
    lines.append('var CDI_SCHEMA = {')
    lines.append('    elements: [')

    # cdi first, then everything else alphabetically
    names = sorted(specs.keys(), key=lambda n: (0 if n == 'cdi' else 1, n))
...
def patch_adapter(js_block):
    """Replace the CDI_SCHEMA block between the markers in cm-adapter.js."""
    ...
    if start_idx == -1 or end_idx == -1:
        ...
        print('Add these comments around the CDI_SCHEMA block:', file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate CDI_SCHEMA for cm-adapter.js from the OpenLCB CDI XSD'
    )
    parser.add_argument(
        '--xsd',
        required=True,
        help='Path to the CDI XSD file  (e.g. ~/Documents/OpenLcb\\ Documents/schema/cdi/1/4/cdi.xsd)'
    )
    parser.add_argument(
        '--patch',
        action='store_true',
        help='Patch cm-adapter.js in-place between the <<CDI_SCHEMA_START>> markers'
    )
